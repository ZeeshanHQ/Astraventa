...
const AstraFlow = () => {
 ...
 setTimeout(() => {
 let responseContent: React.ReactNode = "Mapping orchestration in progress...";
 const lowerText = text.toLowerCase();
 
 if (activeContext === "profile" && (lowerText.includes("name") || lowerText.includes("email"))) {
 setFormData(prev => ({
 ...prev,
 profile: {
 ...prev.profile,
 fullName: lowerText.includes("jane") ? "Jane Doe" : "Alex Mercer",
 email: lowerText.includes("jane") ? "jane@example.com" : "alex.mercer@astraventa.io",
 age: "29",
 role: "Engineer"
 }
 }));
 ...
 <p>I've extracted the core identity parameters and cross-referenced them with your Target Profile schema. Fields populated successfully.</p>
 </div>
 );
 } else {
 responseContent = "I've analyzed the input. To perform live schema mapping across your production systems, please provide your LLM orchestration key in the settings.";
 }
 ...
 value={formData[activeContext][field.id] || ""}
 onChange={(e) => handleInputChange(activeContext, field.id, e.target.value)}
 className={`w-full px-6 py-8 bg-slate-50 border-2 border-slate-200 rounded-2xl focus-visible:ring-4 focus-visible:ring-violet-500/10 focus-visible:border-violet-500 transition-all font-bold text-lg placeholder:text-slate-300 ${
 formData[activeContext][field.id] ? 'bg-violet-50/50 border-violet-200 text-slate-900 animate-in fade-in zoom-in-95 duration-500' : ''
 }`}
 />
 )}
 </motion.div>
 ...
 >
 <div className={`w-14 h-14 shrink-0 rounded-2xl flex items-center justify-center shadow-xl transition-all ${
 msg.role === 'user' ? 'bg-slate-900 text-white' : 'bg-white border-2 border-violet-100 text-violet-600'
 }`}>
 ...
 <div className={`px-8 py-6 rounded-[2.5rem] text-[16px] border shadow-sm ${
 msg.role === 'user' 
