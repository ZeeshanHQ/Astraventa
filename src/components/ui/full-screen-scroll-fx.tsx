...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
 ...
 // Replaced complex GSAP masking with a simple span for cleaner CSS opacity transitions.
 return <span className="fx-featured-content">{text}</span>;
 };
 ...
 // Center title animation is now handled entirely by CSS class toggling (.fx-featured.active)
 // which we already apply in the render loop using (sIdx === index)
 ...
 .fx-fixed-section { height: ${Math.max(1, total + 1)}00vh; position: relative; z-index: 30; }
 ...
 .fx-header {
 grid-column: 1 / 13; align-self: start; padding-top: 14vh; /* Added padding to clear sticky nav */
 font-size: clamp(1.5rem, 5vw, 4rem); line-height: 1; text-align: center; color: var(--fx-text);
 }
 ...
 display: grid; grid-template-columns: 1fr 2fr 1fr; /* L 25% / C 50% / R 25% for more center breathing room */
 align-items: center;
 height: 100%;
 padding: 0 var(--fx-grid-px);
 }
 ...
 .fx-track { will-change: transform; transition: transform 0.3s cubic-bezier(0.25, 1, 0.5, 1); }
 ...
 position: relative;
 font-size: clamp(0.9rem, 1.8vw, 1.4rem); /* reduced size */
 user-select: none;
 cursor: pointer;
 }
 ...
 .fx-center {
 display: grid; place-items: center; text-align: center; height: 60vh; position: relative;
 }
 .fx-featured { position: absolute; opacity: 0; visibility: hidden; width: 100%; transition: opacity 0.5s ease; }
 .fx-featured.active { opacity: 1; visibility: visible; }
 .fx-featured-title {
 margin: 0; color: var(--fx-text);
 font-weight: 900; letter-spacing: -0.01em;
 font-size: clamp(2rem, 6vw, 5.5rem); /* slightly reduced max size */
 }
 ...
 .fx-footer {
 grid-column: 1 / 13; align-self: end; padding-bottom: 8vh; text-align: center; z-index: 10;
 }
